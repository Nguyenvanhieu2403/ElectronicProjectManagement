...
@Component({
  selector: 'app-edit-project-batch',
  templateUrl: './edit-project-batch.component.html',
  styleUrls: ['./edit-project-batch.component.css'],
})
export class EditProjectBatchComponent
  ...
  implements OnInit
{
  ...
  onShowPopup(Id: any) {
    ...
    if (Id > 0) {
      this._service.getProjectBatchById(Id).then(
        (response) => {
          this.itemDetail = response.data;
          this.itemDetail.beginDate = new Date(this.itemDetail.beginDate);
          this.itemDetail.endDate = new Date(this.itemDetail.endDate);
        },
        (error) => {
  ...
  async onInsert() {
    ...
    this._service.updateProjectBatch(model).then(
      (response) => {
